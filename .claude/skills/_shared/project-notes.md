...
| Customer spending activity summary for a time range (BR-55) | `GET /orders/activity-summary/:customerId?range=<last_7_days\|last_30_days\|this_quarter\|year_to_date\|all_time>` — pure read; bucket boundaries built by `b2bridge-api/src/utils/activity-range.ts#buildActivityBuckets` (UTC-based, rolling window, not calendar-anchored for last_7_days/last_30_days). |
| Company members' spending report (BR-55, admin-only "View for" filter) | `GET /company-accounts/:id/members/spending-report` — pure read, all-time aggregate over `company_orders` with `status="APPROVED"`, not affected by the report's time filter. `spend_percentage` is actually share of `remaining_outstanding`, not share of spend. |
