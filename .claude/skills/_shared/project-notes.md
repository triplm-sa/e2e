...
| **A real Net-Term order (BR-55)** | No API shortcut — same "app does not use the theme cart" constraint as above. Chain via UI: `/apps/b2bridge/quick-order` → set qty → "Add to cart" → `/apps/b2bridge/cart` → click **"Order with net terms"** (separate button from "Checkout", only appears when the buyer is NT-eligible) → `/apps/b2bridge/cart-info` (shipping address form, often pre-filled; a Vietnam address needs a **valid postal code** — `01001` is rejected, `100000` works) → "Continue to shipping option" → "Checkout" → lands on `/apps/b2bridge/checkout-success`. Order appears in `/report` within seconds (no observed indexing lag). |
| Draft order with a specific payment-terms template | `draftOrderInput.paymentTerms.paymentTermsTemplateId = "gid://shopify/PaymentTermsTemplate/<n>"` — these ids are **Shopify's own fixed global ids**, not app-defined: `1`=Due on receipt, `2`=Net 7, `3`=Net 15, `4`=Net 30, `5`=Net 60, `6`=Net 90, `7`=Custom/fixed-date (see `ELIGIBLE_OPTIONS` in `ReportCreditCard.tsx`/`NtForm.tsx`). Shopify computes `dueAt` itself from order-creation time + template duration — **there is no way to backdate it**, so an already-overdue order cannot be created directly; it only becomes overdue once real time passes its due date. |
| ⚠️ **Creating a 2nd company account always sends a REAL Shopify customer-invite email** | `CompanyAccountService.createShopifyCustomer` (`company-account.service.ts:492`) hardcodes `send_email_invite: true` on the Shopify Admin `customers.json` POST. No app-level setting blocks it — `email_template.*.status` only gates B2Bridge's *own* transactional emails, not this. Get explicit tester sign-off (and a real email they control) before creating a second company for any IDOR/cross-tenant case. |
