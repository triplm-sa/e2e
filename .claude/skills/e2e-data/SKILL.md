...
3. **Prefer reaching the state yourself over asking a human for it.** Check `../_shared/project-notes.md` for chains already known for this app, then follow `../_shared/references/automation-ladder.md`: when an endpoint can produce the entity, express it as `phase: setup` steps in `cases.yaml` (with `phase: teardown` cleanup) rather than recording it as a manual prerequisite. Reserve manual preparation for entities no rung of the ladder can create.
