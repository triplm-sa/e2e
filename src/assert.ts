...
export function evalExpect(spec: ExpectSpec, status: number, body: unknown): { passed: boolean; detail: string } {
  ...
  if (spec.status !== undefined && spec.status !== status) {
    problems.push(`status: expected ${spec.status}, got ${status}`);
  }
  for (const [path, want] of Object.entries(spec.bodyMatch ?? {})) {
    const got = getPath(body, path);
    if (JSON.stringify(got) !== JSON.stringify(want)) {
      problems.push(`body.${path}: expected ${JSON.stringify(want)}, got ${JSON.stringify(got)}`);
    }
  }
  return problems.length
    ? { passed: false, detail: problems.join("; ") }
    : { passed: true, detail: `status ${status} + ${Object.keys(spec.bodyMatch ?? {}).length} bodyMatch ok` };
}
