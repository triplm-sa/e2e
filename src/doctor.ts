...
console.log("[1] Google Chrome (required when browser targets exist)");
...
try {
  ...
  bad(`Config error: ${(e as Error).message}`);
  finish();
}

console.log("[3] Targets");
const entries = Object.entries(cfg.targets ?? {}) as [string, Target][];
if (!entries.length) bad("No targets defined.");
const hasBrowser = entries.some(([, t]) => t.kind === "browser");
if (hasBrowser && !chrome) bad("Google Chrome not found but a browser target is configured.");
else if (!chrome) warn("Google Chrome not found — API-only runs are still available.");
